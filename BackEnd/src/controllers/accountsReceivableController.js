 ...
 */
    ...
    const paid = await accountsReceivableModel.readPaid(id);
    ...
    if (paid[0].Paid != reqBody.paid) {
      if (reqBody.paid === true) {
        ...
        const cashFlow = { Origin: 'Contas a Receber', Description: `Id: ${id}. Selecionado como pago.`, Amount: reqBody.amount, Accumulated: sun }
      ...
      if (reqBody.paid === false) {
        let accumulated = await cashFlowModel.readBalance();
        let sun = accumulated[0].Accumulated - reqBody.amount;
        const cashFlow = { Origin: 'Contas a Receber', Description: `Id: ${id}. Desmarcado como pago.`, Amount: reqBody.amount, Accumulated: sun }
        const result = await cashFlowModel.create(cashFlow);
      }
    };
